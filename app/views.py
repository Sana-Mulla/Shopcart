...
def show_cart(request):
    if request.user.is_authenticated:
       ...
       # this will give u queryset of cart products
        amount = 0.0
        shipping_amount  = 50.00
        total_amount = 0.0
        cart_product  = [ p for p in Cart.objects.all() if p.user==request.user]#this will give list of cart product
...
def payment_done(request):
    ...
    cart = Cart.objects.filter(user=user)
    for c in cart:
        OrderPlaced.objects.create(user=user,customer=customer,product=c.product,quantity=c.quantity).save()
        c.delete()
...
def mobile(request, data=None):
    if data==None:
        mobiles = Product.objects.filter(category='M')
    elif data=='Samsung' or data=='OnePlus' or data=='Redmi':
